...
auto BeDirectionalLightPass::Setup(
    ...
    lightBufferDescriptor.ByteWidth = sizeof(DirectionalLightBufferGPU);
...
auto BeDirectionalLightPass::Execute(const ComPtr<ID3D11DeviceContext>& context) -> void {
    if (!_graph || !_shader) {
        return;
    }
    ...
    //!< Update and bind light constant buffer
    {
        DirectionalLightBufferGPU lightBuffer(_lightData);
        D3D11_MAPPED_SUBRESOURCE mappedResource;
        Utils::Check << context->Map(_lightBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
        memcpy(mappedResource.pData, &lightBuffer, sizeof(DirectionalLightBufferGPU));
...
auto BeDirectionalLightPass::GetInputResources() const -> std::vector<std::string> {
    return {"DepthBuffer", "GBuffer0", "GBuffer1", "GBuffer2"};
}

auto BeDirectionalLightPass::GetOutputResources() const -> std::vector<std::string> {
